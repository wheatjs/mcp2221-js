...
const string = Object.create(ref.types.CString)
string.name = 'WCString';
string.get = function get(buf, offset) {
  var _buf = buf.readPointer(offset);
  if (_buf.isNull()) {
    return null;
  }
  var stringBuf = _buf.reinterpretUntilZeros(64);
  return exports.toString(stringBuf);
};
string.set = function set(buf, offset, val) {
  var _buf = val; // val is a Buffer? it better be \0 terminated...
  if ('string' == typeof val) {
    _buf = setter.convert(val + '\0');
  }
